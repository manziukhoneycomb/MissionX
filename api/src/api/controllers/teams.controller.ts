...
import {
    ITeamsService,
    TEAMS_SERVICE,
} from '../../application/teams/interfaces/teams.service.interface';
...
export class TeamsController {
    ...
    @Post(':id/members')
    @HttpCode(HttpStatus.NO_CONTENT)
    @ApiOperation({ summary: 'Add member to team', description: 'Adds a user to the team' })
    @ApiParam({
        name: 'id',
        description: 'Team ID',
        example: '123e4567-e89b-12d3-a456-426614174000',
    ...
    @ApiUnauthorizedResponse({ description: 'Unauthorized' })
    @ApiForbiddenResponse({ description: 'Forbidden - user must belong to same tenant' })
    async addMember(
        @Param('id') id: string,
